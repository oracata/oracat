package com.oracat.model;

public class ErpCustom {
    private String shengfen     ;
    private String chengshi     ;
    private String quyufl       ;
    private String kehulb       ;
    private String wldwid     ;
    private String wldwbh       ;
    private String wldwname     ;
    private String lxdh         ;
    private String lxr          ;
    private String ds_lxr       ;
    private String ds_lxdh      ;
    private String is_dssc      ;
    private String shouhr       ;
    private String shr_lxdh     ;

    public String getShengfen() {
        return shengfen;
    }

    public void setShengfen(String shengfen) {
        this.shengfen = shengfen;
    }

    public String getChengshi() {
        return chengshi;
    }

    public void setChengshi(String chengshi) {
        this.chengshi = chengshi;
    }

    public String getQuyufl() {
        return quyufl;
    }

    public void setQuyufl(String quyufl) {
        this.quyufl = quyufl;
    }

    public String getKehulb() {
        return kehulb;
    }

    public void setKehulb(String kehulb) {
        this.kehulb = kehulb;
    }

    public String getWldwid() {
        return wldwid;
    }

    public void setWldwid(String wldwid) {
        this.wldwid = wldwid;
    }

    public String getWldwbh() {
        return wldwbh;
    }

    public void setWldwbh(String wldwbh) {
        this.wldwbh = wldwbh;
    }

    public String getWldwname() {
        return wldwname;
    }

    public void setWldwname(String wldwname) {
        this.wldwname = wldwname;
    }

    public String getLxdh() {
        return lxdh;
    }

    public void setLxdh(String lxdh) {
        this.lxdh = lxdh;
    }

    public String getLxr() {
        return lxr;
    }

    public void setLxr(String lxr) {
        this.lxr = lxr;
    }

    public String getDs_lxr() {
        return ds_lxr;
    }

    public void setDs_lxr(String ds_lxr) {
        this.ds_lxr = ds_lxr;
    }

    public String getDs_lxdh() {
        return ds_lxdh;
    }

    public void setDs_lxdh(String ds_lxdh) {
        this.ds_lxdh = ds_lxdh;
    }

    public String getIs_dssc() {
        return is_dssc;
    }

    public void setIs_dssc(String is_dssc) {
        this.is_dssc = is_dssc;
    }

    public String getShouhr() {
        return shouhr;
    }

    public void setShouhr(String shouhr) {
        this.shouhr = shouhr;
    }

    public String getShr_lxdh() {
        return shr_lxdh;
    }

    public void setShr_lxdh(String shr_lxdh) {
        this.shr_lxdh = shr_lxdh;
    }

}
